...
The streaming architecture already exists in pieces. The work should focus on safely exposing and enabling the existing partial-result path rather than building a parallel voice input pipeline.
...
- Do not rewrite `AudioRecognizer` threading or the `WhisperGGML` JNI boundary for this run.
...
- Do not introduce a separate parallel voice input pipeline when the existing partial-result path can be enabled.
